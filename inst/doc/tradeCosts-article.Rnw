...
sample shown above: \texttt{period} is the period during which the
trade was executed, in this case a calendar trade day; \texttt{id} is
a unique security identifier; \texttt{side} must be one of \texttt{B}
(buy), \texttt{S} (sell), \texttt{C} (cover) or \texttt{X} (short
sell); \texttt{exec.qty} is the number of shares executed; and
\texttt{exec.price} is the price per share of the execution.
...
\texttt{prior.close} is the security's prior period closing price.
...
\texttt{tradeCostsResults}, upon which we can call the
...
The first section of the report above provides high-level summary
information.  The total unsigned market value of trades for March 1
was around \$1.3mm.  Relative to VWAP, these trades cost -\$6,491,
indicating that overall the trades were executed at a level better
than VWAP.  This total cost is the sum of the signed cost of each
trade relative to the benchmark price.  As a percentage of total
executed market value, this set of trades cost -51 bps less than VWAP.
...
\texttt{id}, \texttt{period}, \texttt{side}, \texttt{exec.price} and
\texttt{exec.qty}.  The final section shows a count for each type of
\texttt{NA} in the input data.  Rows in the input data with
\texttt{NA}'s in any of these columns are removed before the analysis
is performed and reported here.
...
is either specific to the period of the trade or fixed at the decision
price.
...
How should the trader calculate the cost of these trades?  One way is
to calculate the cost for each trade separately relative to a benchmark
price such as \texttt{vwap}, exactly as in the last example.  In this
case, the cost of each trade in PGIC would be calculated relative to
VWAP in each period and then added together.  However, this method
would ignore the cost associated with spreading out the sale over several
days.  If the price of the stock had been falling over the four days
of the sale, as it appears to in this example, successive trades
appear less attractive when compared to the price at the time of the
decision.  The trader can capture this cost by grouping the four short
sales into a \emph{batch} and comparing the execution price of each
trade to the batch's original decision price.
...
This analysis covers almost \$50mm of executions from March 1 to March
30, 2007.  Relative to decision price, the trades cost \$587,148, or
1.23\% of the total executed market value.
...
\$182,707.  Below is a list of each trade and its associated cost that
contributed to this batch's total cost.
...
<<echo=FALSE>>=
subset(result.batched@executions, id %in% "59862K10",
       select = c("period", "id", "side", "exec.qty", "exec.price",
...
Here \texttt{execution.cost} denotes the dollar cost of the trade
relative to the benchmark price, and \texttt{pct.exe.cost} reflects
that cost as a percent of the total executed dollar amount of the
trade.  The decision price for the batch is \$5.97, the prior closing
price on March 13.  Note that the benchmark price,
\texttt{decision.price}, for each trade in this batch is \$5.97.
Because the price of PGIC was falling over this time period, later
trades in the batch are more than five times more costly than the
first trade relative to the decision price.
...
The simplest plot, \texttt{time.series.bps} is a time series of total
trade costs in basis points over each period:
